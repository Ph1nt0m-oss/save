...
# ==================== FASTAPI APP ====================
# iter124 — Lifespan handler (remplace @app.on_event deprecated dans FastAPI ≥0.110).
# Voir _lifespan() en bas du fichier où on attache ensure_indexes + tasks périodiques.
...
# iter116 — slice 5a : /community-bots/* extraits dans routes/community_bots_routes.py
from routes.community_bots_routes import build_community_bots_router  # noqa: E402
app.include_router(
    build_community_bots_router(
        db,
        verify_signed=_verify_signed,
        require_creator_signature=_require_creator_signature,
...
# iter116 — slice 5b : /devices/* (17 endpoints) extraits dans routes/devices_routes.py
from routes.devices_routes import build_devices_router  # noqa: E402
app.include_router(
    build_devices_router(
        db,
        compute_key_id=compute_key_id,
        new_nonce=new_nonce,
        verify_signature=verify_signature,
...
# iter118 — slice 5f : /ideas/* (7 endpoints) extraits dans routes/ideas_routes.py
from routes.ideas_routes import build_ideas_router  # noqa: E402
app.include_router(
    build_ideas_router(
        db,
        verify_signed=_verify_signed,
...
# iter118 — slice 5g : /webauthn/* (6 endpoints) extraits dans routes/webauthn_routes.py
from routes.webauthn_routes import build_webauthn_router  # noqa: E402
app.include_router(
...
# iter122 — slice 5p : /preview/* (3 endpoints) — preview HTML pages
from routes.preview_routes import build_preview_router  # noqa: E402
app.include_router(build_preview_router(db), prefix="/api")

# iter122 — slice 5q : /voice/transcribe (1 endpoint) — Whisper STT
from routes.voice_routes import build_voice_router  # noqa: E402
app.include_router(
    build_voice_router(db, get_current_user=get_current_user, logger=logger),
    prefix="/api",
)

# iter122 — slice 5r : /projects/* + /share/{slug} (8 endpoints) — projects CRUD + sharing
from routes.projects_routes import build_projects_router  # noqa: E402
app.include_router(
...
# iter124 — slice 5v : /chat/analyze-attachment + /chat/models + /chat/export-{ipynb,docx} (4 endpoints, ~400 L)
from routes.chat_exports_routes import build_chat_exports_router  # noqa: E402
app.include_router(
    build_chat_exports_router(
        db,
        get_current_user=get_current_user,
        logger=logger,
        sanitize_filename=_sanitize_filename,
        analyze_pdf=_analyze_pdf,
        analyze_docx=_analyze_docx,
        analyze_xlsx=_analyze_xlsx,
        analyze_pptx=_analyze_pptx,
        analyze_sqlite=_analyze_sqlite,
        analyze_image_with_vision=_analyze_image_with_vision,
    ),
    prefix="/api",
)
