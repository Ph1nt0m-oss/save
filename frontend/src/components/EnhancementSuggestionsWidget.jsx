...
const KIND_META = {
  feature: { icon: Sparkles, color: 'violet', label: 'Fonctionnalité' },
  fix: { icon: Wand2, color: 'amber', label: 'Correction' },
  design: { icon: Palette, color: 'rose', label: 'Design' },
  integration: { icon: Plug, color: 'cyan', label: 'Intégration' },
  performance: { icon: Zap, color: 'emerald', label: 'Performance' },
};
            ...
            return (
              <motion.div
                ...
                className={`relative bg-[#0A0A0A] border rounded-sm p-3 cursor-pointer transition-all ${
                  isSelected ? `border-${meta.color}-400/60 shadow-[0_0_0_1px_rgba(168,85,247,0.2)]` : 'border-white/10 hover:border-white/20'
                }`}
              >
                {/* Thumbnail or icon */}
                <div className="flex items-start gap-2.5 mb-2">
                  {s.thumbnail_url ? (
                    ...
                    <div className={`w-10 h-10 rounded-sm flex items-center justify-center flex-shrink-0 bg-${meta.color}-500/10 border border-${meta.color}-400/30`}>
                      <Icon className={`w-4 h-4 text-${meta.color}-300`} />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <span className={`text-[9px] uppercase tracking-widest text-${meta.color}-300 font-['Chivo'] font-bold`}>
                      {meta.label}
                    </span>
                ...
                {isSelected && (
                  <motion.div
                    ...
                    className={`absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-${meta.color}-500 border-2 border-[#0A0A0A] flex items-center justify-center`}
                  >
